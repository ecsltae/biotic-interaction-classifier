...
# NOTE: the manuscript (classifier/manuscript/biotic_interaction_classifier.tex)
# reads figures from classifier/manuscript/figures/, not from here directly.
# After regenerating any of fig_01_version_f1, fig_02_model_evolution,
# fig_distribution_mismatch, fig_manuscript_ci, fig_threshold_curve, copy the
# updated PNG into manuscript/figures/ before recompiling the paper.
...
def fig_ablation_table():
    ...
    rows = [
        ["basic_a05",                 "basic",      "0.5", "cold", "0 ep", "0.861", "0.884", "0.840"],
        ["typed_a05",                 "typed",      "0.5", "cold", "0 ep", "0.794", "0.897", "0.712"],
        ["full_a03",                  "full",       "0.3", "cold", "0 ep", "0.858", "0.892", "0.826"],
        ["full_typed_a05",            "full_typed", "0.5", "cold", "0 ep", "0.840", "0.879", "0.804"],
    ...
    row_colors = [
        GOOD20,           # basic_a05 is the best cold-start scheme
        WHITE, LGREY, WHITE,
        LGREY, WHITE,
        LGREY,            # full_typed_a05_ner2 (retained for HOST/PATHOGEN typing, not best cold-start)
        WHITE,
    ...
    ax.set_title("Multi-task Ablation — Full Results, 500-sentence Test Set\n"
                 "★ = champion (warm-start, 0 NER pre-train epochs)    "
                 "‡ = checkpoint from earlier test-set version    ── = reference baselines",
                 pad=14)
    render_table(ax, rows, cols, row_colors,
                 col_widths=[3.4, 1.8, 0.5, 0.7, 1.0, 0.9, 0.8, 0.8],
...
# ══════════════════════════════════════════════════════════════════════════════
def fig_ner_schemes():
    # Updated to 500-sentence test set (manuscript Table 2). Cold-start F1 does
    # NOT rank monotonically by scheme richness: basic is the best cold-start
    # scheme, typed is the worst. full_typed is carried forward regardless
    # because its HOST/PATHOGEN typing is required for the KG use case, and
    # it is the scheme that reaches 0.874 under warm-start (the overall best).
    cols = ["Scheme", "Labels included", "What is tagged", "# labels", "Cold-start F1"]
    rows = [
        ["basic ★",     "O, B/I-SPECIES",
         "Any species mention\n(no role distinction)", "3", "0.861"],
        ["typed",       "O, B/I-HOST, B/I-PATHOGEN",
         "Role-distinguished species\n(host vs pathogen)", "5", "0.794"],
        ["full",        "O, B/I-SPECIES, B/I-INT",
         "Any species\n+ interaction verbs", "5", "0.858"],
        ["full_typed †","O, B/I-HOST, B/I-PATHOGEN, B/I-SPECIES, B/I-INT",
         "Role-distinguished species\n+ interaction verbs", "9", "0.840"],
    ]
    row_colors = [GOOD20, LGREY, WHITE, WARN15]

    fig, ax = plt.subplots(figsize=(13, 4.2))
    ax.set_title("NER Label Schemes  —  B = Beginning of span, I = Inside span\n"
                 "★ = best cold-start F1     † = retained for HOST/PATHOGEN typing, reaches 0.874 with warm-start",
                 pad=12)
    render_table(ax, rows, cols, row_colors,
